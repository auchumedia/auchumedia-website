  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }} className="nav-desktop">
          ...
          >
            <button onClick={() => setDdOpen(!ddOpen)} style={{
              ...
              background: ddOpen ? 'rgba(255,255,255,0.05)' : 'transparent',
            ...
            {ddOpen && (
              <div style={{
                ...
                background: '#0f0f0f', border: '0.5px solid rgba(255,255,255,0.12)',
                borderRadius: '10px', overflow: 'hidden',
                boxShadow: '0 20px 60px rgba(0,0,0,0.5)'
              }}>
                <Link to="/athletes" onClick={() => setDdOpen(false)} style={{
                  display: 'flex', alignItems: 'center', gap: '14px',
                  padding: '18px 20px', borderBottom: '0.5px solid rgba(255,255,255,0.07)',
                  transition: 'background 0.15s'
                }}
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.04)'}
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                >
                  <div style={{ width: '40px', height: '40px', borderRadius: '8px', background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.65)" strokeWidth="1.8"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#fff', marginBottom: '3px' }}>{fr ? 'Pour athlètes' : 'For athletes'}</div>
                    <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)' }}>{fr ? 'Personal branding · Partnerships · Post-carrière' : 'Personal branding · Partnerships · Post-career'}</div>
                  </div>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#fff', marginBottom: '3px' }}>{fr ? 'Pour entreprises' : 'For businesses'}</div>
                    <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)' }}>Always-On · Meta Ads · CRM</div>
                  </div>
                  <span style={{ color: 'rgba(255,255,255,0.2)' }}>→</span>
                </Link>
              </div>
      ...
      <div style={{
        ...
        background: '#0d0d0d', borderBottom: '0.5px solid rgba(255,255,255,0.07)',
        ...
        <div style={{ padding: '16px 20px 24px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.25)', padding: '12px 0 8px' }}>Services</div>
          {[
            { to: '/athletes', label: fr ? 'Pour athlètes' : 'For athletes' },
            { to: '/entreprises', label: fr ? 'Pour entreprises' : 'For businesses' },
          ].map(item => (
            <Link key={item.to} to={item.to} onClick={() => setMobileOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', fontWeight: 600, color: 'rgba(255,255,255,0.45)', padding: '10px 0 10px 14px', borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
